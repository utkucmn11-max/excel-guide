...
if page == "Ana Sayfa":
    ...
    st.markdown("Bu platform, Excel dünyasında ustalaşmanız için tasarlandı.")
    st.markdown("Aşağıdaki kategorilerden birini seçerek öğrenmeye başlayabilirsiniz.")
...
elif page == "İleri Seviye İpuçları":
    st.header("💡 Uzman İpuçları")
    st.write("Excel'de hızlanmak için bu özellikleri mutlaka öğrenin:")
    st.markdown("- **Pivot Tablolar:** Büyük verileri özetlemek için kullanılır.")
    st.markdown("- **Düşeyara (VLOOKUP):** Tablolar arası veri çekmek için kullanılır.")
    st.markdown("- **F4 Tuşu:** Formüllerde hücre sabitleme ($A$1) işlemini saniyeler içinde yapar.")
